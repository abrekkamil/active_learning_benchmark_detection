...
from src.rl_active_learning_SDNET_patched import ActiveLearningSystemRL
...
config_path = SCRIPT_DIR / "dynamic_rl_active_learning_sdnet2018_patched.yaml"
    ...
    for query_size in query_sizes:
        for cold_start in cold_start_strategies:
            ...
            experiment_parameters = (
                f"_i{int(initial_labeled_size * 100)}pct_"
                f"dynamic_continuous_q"
            )
            ...
            config.model_name = "resnet101_classification"
            ...
            config.budget_mode: "continuous"
            config.budget_ratio_min: 0.001
            config.budget_ratio_max: 0.02

            config.experiment_name = (
                f"Dynamic_RAL_resnet101_SDNET2018_c30_e5_ie15_b0.001-2_"
            ...
            if config.budget_mode == 'discrete':
                print(f"budget_mode: {config.budget_mode} and Budget options:{config.budget_options}")
            else:
                print(f"budget_mode: {config.budget_mode} and Budget is between:{config.budget_ratio_min} and {config.budget_ratio_max}pct")
