...
for initial_labeled_size in initial_labeled_sizes:
    for query_size in query_sizes:
        ...
        config.cold_start_strategy = "weak_supervision"
        config.model_name = 'maskrcnn'
        config.dynamic_query_size = False
        config.use_wandb = False
        config.pool = True
        config.experiment_name = 'Active_Learning_instance_masonry'
        config.al_cycles = 10
...
config_path = "../Reinforcement_Active_Learning/rl_ins_segmentation_masonry.yaml"
config = ActiveLearningConfig.from_yaml(config_path)
for initial_labeled_size in initial_labeled_sizes:
    for query_size in query_sizes:
        # Optional overrides for interactive testing
        ...
        config.use_wandb = False
        config.pool = True
        config.experiment_name = 'Reinforcement_Active_Learning_instance_masonry'
        config.oracle_epochs = 50
        config.al_cycles = 10
        config.initial_labeled = initial_labeled_size
        config.query_size = query_size
        config.epochs_per_cycle = 10
