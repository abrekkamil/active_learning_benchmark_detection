...
def get_feature_cache_path(args):
    ...
    return os.path.join(cache_dir, "sewerml_resnet18_{}".format(args.scale_size))
...
class ActiveLearningPipeline:
    ...
    def _compute_pos_weight_from_indices(self, indices, eps=1e-6):
        # Load labels only for the selected indices via DataLoader
        ...
        # Clip to avoid extreme weights on very rare classes
        pos_weight = np.clip(pos_weight, a_min=1.0, a_max=50.0)
    ...
    def cold_start(self):
        ...
        # Selection report path (day folder + time tag)
        ...
        selected = apply_cold_start(
            ...
            secondary_strategy=getattr(self.args, 'secondary_strategy', 'stratified'),
            save_selection_path=selection_report_path,
        )
        ...
        self.logger.info(
            "Selection report saved to: {}".format(selection_report_path)
        )

        self.history["cold_start_strategy"]    = strategy
        self.history["cold_start_labeled"]     = len(self.labeled_indices)
        self.history["cold_start_report_path"] = selection_report_path
        self.save_results()
    ...
    # ----------------------------------------------------------
    # CIW-recall priority vector
    # ----------------------------------------------------------
    def _ciw_recall_priority(self, val_loader):
        """
        Returns a priority weight per class: classes that are
        high-CIW but low-recall get the highest weight.
 
        priority[c] = CIW[c] * (1 - recall[c])
 
        When no per-class metrics are available yet (first cycle),
        returns uniform weights so the first query is unbiased.
        """
        class_names = val_loader.dataset.LabelNames
        ciw_arr     = np.array(
            [self.CIW.get(c, 0.0) for c in class_names], dtype=np.float32
        )
 
        if not self.last_per_class_metrics:
            # No metrics yet: uniform priority
            priority = ciw_arr / (ciw_arr.sum() + 1e-8)
        else:
            recall_arr = np.array(
                [self.last_per_class_metrics.get(c, {}).get("recall", 0.0)
                 for c in class_names],
                dtype=np.float32
            )
            # High CIW + low recall = high priority
            raw      = ciw_arr * (1.0 - recall_arr)
            priority = raw / (raw.sum() + 1e-8)
 
            # Log which classes are driving the priority bonus
            top3 = np.argsort(priority)[::-1][:3]
            self.logger.info(
                "CIW-recall priority top-3: {}".format(
                    ", ".join(
                        "{} (CIW={:.2f} R={:.3f} pri={:.4f})".format(
                            class_names[i],
                            ciw_arr[i],
                            1.0 - priority[i] / (ciw_arr[i] + 1e-8),
                            priority[i]
                        )
                        for i in top3
                    )
        ...
        return torch.tensor(priority, dtype=torch.float32, device=self.device)
 
    # ==========================================================
    # RL Query  (entropy filter + policy + CIW-recall bonus)
    # ==========================================================
    def query(self, budget, val_loader=None):
        """
        Three-stage selection:
 
        1. Entropy filter  — keep top candidate_ratio% of unlabeled pool
           by prediction entropy (same as before).
 
        2. CIW-recall bonus — for each candidate, estimate which weak
           high-CIW classes it likely belongs to using the oracle model
           predictions, then add a bonus proportional to the CIW-recall
           priority of those classes. This steers the policy toward
           samples that will help the classes that matter most.
 
        3. Policy sampling  — multinomial over final logits (policy
           logits + bonus), log-prob and entropy used for REINFORCE.
        """
        ...
        chunks = []
        # Also collect oracle probabilities for the bonus computation
        prob_chunks = []
        with torch.no_grad():
            for batch in loader:
                imgs = batch['image'].to(self.device)
                chunks.append(self._compute_state(imgs))
                # Oracle probs [B, C] used for bonus
                prob_chunks.append(
                    torch.sigmoid(self.oracle_model(imgs)).cpu()
                )
 
        states      = torch.cat(chunks,      dim=0).detach()   # [N, state_dim]
        oracle_probs = torch.cat(prob_chunks, dim=0)            # [N, C]
 
        # --------------------------------------------------
        # Stage 1: entropy-based candidate filtering
        # --------------------------------------------------
        entropy_scores  = states[:, -3]
        candidate_ratio = getattr(self.args, 'candidate_ratio', 0.2)
        top_k           = max(1, min(
            int(candidate_ratio * len(entropy_scores)),
            len(entropy_scores)
        ))
        _, cand_idx      = torch.topk(entropy_scores, top_k)
        cand_idx_cpu     = cand_idx.cpu()
        candidate_states = states[cand_idx]
        candidate_probs  = oracle_probs[cand_idx_cpu].to(self.device)  # [K, C]
        candidate_pool   = [self.unlabeled_indices[i] for i in cand_idx_cpu.tolist()]
 
        # --------------------------------------------------
        # Stage 2: CIW-recall bonus
        # Bonus[k] = sum_c( priority[c] * prob[k,c] )
        # = expected priority of the sample under oracle predictions.
        # Samples likely to contain high-priority (high CIW, low recall)
        # classes get a higher bonus.
        # --------------------------------------------------
        priority = None
        if val_loader is not None:
            priority = self._ciw_recall_priority(val_loader)   # [C]
 
        # --------------------------------------------------
        # Stage 3: policy forward
        # --------------------------------------------------
        global_state    = candidate_states.mean(dim=0)
        image_logits, _ = self.policy(candidate_states, global_state)
        # image_logits: [K, 1]
 
        ciw_bonus_weight = getattr(self.args, 'ciw_bonus_weight', 0.3)
 
        if priority is not None:
            # bonus[k] in [0, 1] — how much this sample targets weak classes
            bonus = (candidate_probs * priority.unsqueeze(0)).sum(dim=1,
                     keepdim=True)                              # [K, 1]
            # Normalise bonus to same scale as logits
            bonus = (bonus - bonus.mean()) / (bonus.std() + 1e-8)
            combined_logits = image_logits + ciw_bonus_weight * bonus
        else:
            combined_logits = image_logits
 
        budget      = max(1, min(budget, len(candidate_pool)))
        image_probs = F.softmax(
            combined_logits.squeeze() / self.policy_temp, dim=0
        ).clamp_min(1e-12)
 
        selected_pos     = torch.multinomial(
            image_probs, num_samples=budget, replacement=False
        )
        ...
        self.logger.info(
            "Query: {} selected from {} candidates (pool {}) | "
            "ciw_bonus_weight={:.2f}".format(
                len(selected_indices), len(candidate_pool),
    ...
    def evaluate_main_model(self, val_loader):
        ...
        f1_normal = f1_score(y_true_normal, y_pred_normal, zero_division=0)
        p_normal  = precision_score(y_true_normal, y_pred_normal, zero_division=0)
        r_normal  = recall_score(y_true_normal, y_pred_normal, zero_division=0)

        n_normal_true = int(y_true_normal.sum())
        n_normal_pred = int(y_pred_normal.sum())
        ...
        metrics = dict(
            ...
            p_normal=p_normal,           
            r_normal=r_normal,            
            n_normal_true=n_normal_true,  
            n_normal_pred=n_normal_pred,  
            per_class=per_class,
        )
        self.logger.info(
            "m-F1: {:.4f} | M-F1: {:.4f} | mAP: {:.4f} | CIW-F2: {:.4f} | F1-Normal: {:.4f} ({}/{} pos)".format(
            micro_f1, macro_f1, mAP, ciw_f2, f1_normal, n_normal_pred, n_normal_true
                )
            )
        self.last_per_class_metrics = per_class
        return metrics
    ...
    def run_cycle(self, val_loader, budget: int, cycle_epochs: int):
        ...
        new_indices, log_prob_sum, entropy = self.query(budget, val_loader=val_loader)
    ...
    def _log_metrics(self, epoch, train_time, train_metrics, eval_metrics):
        ...
        # ---------------------------------
            ...
            f1_normal = eval_metrics.get("f1_normal", 0)
            ...
            self.history.setdefault("val_f1_normal", []).append(f1_normal)
