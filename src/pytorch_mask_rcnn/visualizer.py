...
class Visualizer:
    ...
    def masks_to_polygons(masks):
        ...
        for mask in masks:
            ...
            rle['counts'] = rle['counts'].decode('utf-8')  # pycocotools string fix
            poly = mask_util.frPyObjects(rle, mask.shape[0], mask.shape[1])
            ...
            if masks is not None:
                for i in range(len(masks)):
                    try:
                        ...
                        # Convert GenericMask binary mask to polygons using OpenCV
                        mask = masks[i].mask.astype(np.uint8)  # (H, W)
                        contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                        for contour in contours:
                            if contour.shape[0] >= 3:  # valid polygon
                                self.draw_polygon(contour.reshape(-1, 2), color, alpha=alpha)
